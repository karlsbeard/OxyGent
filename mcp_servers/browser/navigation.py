...
from .core import (
    mcp, _ensure_page, _set_operation_status, 
    _verify_data_ready, check_dependencies
)
...
async def browser_navigate(url: str = Field(description="要导航到的网页URL"),
    ...
    try:
        ...
        # 获取页面ID
        from .core import _pages, _login_in_progress, _original_urls
        ...
        if page_id and not _login_in_progress.get(page_id, False):
            ...
            if await _check_login_required(page):
                ...
                try:
                    # 优先使用域名特定配置进行登录
                    login_success = False
                    from .login import _auto_login_jd, _auto_login_with_config, LOGIN_DOMAIN_CONFIGS, _detect_2fa_required, _handle_2fa_authentication
                    ...
                    if login_success:
                        ...
                        # 如果有原始URL，则重新导航到该URL
                        if page_id in _original_urls and _original_urls[page_id] != page.url:
                            ...
                            try:
                                ...
                                if final_url == original_url or final_url.startswith(original_url) or not await _check_login_required(page):
                                    print(f"成功访问目标页面: {final_url}")
                                    # 清除原始URL
                                    del _original_urls[page_id]
                                else:
                                    print(f"导航到原始URL后页面仍然需要登录，可能需要不同的登录方式")
                                    
                                    # 检查重定向后的页面是否仍然需要登录
                                    if await _check_login_required(page):
                                        print("尝试对重定向后的页面进行登录...")
                                        # 尝试对重定向后的页面进行登录
                                        redirect_domain = await _get_domain_from_url(final_url)
                                        second_login_success = False
                                        
                                        if redirect_domain in LOGIN_DOMAIN_CONFIGS:
                                            print(f"使用域名 {redirect_domain} 的特定配置进行登录")
                                            second_login_success = await _auto_login_jd(page)
                                        else:
                                            print("使用通用登录方法")
                                            second_login_success = await _auto_login_with_config(page)
                                ...
                                print(f"导航到原始URL时发生错误: {str(e)}，保留原始URL记录")
                    else:
                        print("自动登录失败")
                finally:
                    # 无论登录成功与否，都标记登录处理完成
        ...
        if response:
            ...
            if needs_2fa:
                ...
                if auth_result_var and isinstance(auth_result_var, dict):
                    result["two_factor_auth_status"] = auth_result_var.get("status", "unknown")
                    result["two_factor_auth_message"] = auth_result_var.get("message", "")
                    if 'screenshot_path' in auth_result_var:
                        result["auth_screenshot_path"] = auth_result_var.get("screenshot_path", "")
                    if 'action_required' in auth_result_var:
                        result["action_required"] = auth_result_var.get("action_required", "")

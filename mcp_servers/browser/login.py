...
from .core import (
    mcp, _pages, _login_in_progress, _original_urls, 
    _ensure_page, _set_operation_status, _verify_data_ready,
...
async def _auto_login_with_config(page, domain=None, username=None, password=None, env_username_key="JD_ERP_USERNAME", env_password_key="JD_ERP_PASSWORD"):
    """
    使用配置自动登录
    ...
    try:
        ...
        if not login_username or not login_password:
            print(f"未提供用户名或密码，且未找到环境变量 {env_username_key} 或 {env_password_key}")
            return False
        ...
        for pattern in login_patterns:
            try:
                # 尝试查找用户名输入框
                username_element = await page.query_selector(pattern["username_selector"])
                if not username_element:
                    continue
                ...
                # 尝试查找密码输入框
                password_element = await page.query_selector(pattern["password_selector"])
                if not password_element:
                    continue
...
async def browser_auto_login(url: str = Field(description="要登录的网站URL"),
                    ...
                    if not password_element:
                        ...
                        if not submit_button:
                            ...
                            if needs_2fa:
                                print("检测到需要二次认证")
                                # 处理二次认证
                                auth_result = await _handle_2fa_authentication(page, wait_for_2fa)
                                ...
                                if auth_status == "pending_2fa":
                                    ...
                                    result["action_required"] = auth_result.get("action_required", "请完成二次认证")
                                elif auth_status == "success":
                                    result["status"] = "success"
                                    result["message"] = "登录成功，并已处理二次认证"
                                else:
                                    result["status"] = auth_status
                                    result["message"] = auth_result.get("message", "二次认证处理状态未知")

...
class TestBrowserServer(unittest.TestCase):
    ...
    def test_server_imports(self):
        ...
        # 获取server.py的绝对路径
        server_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'mcp_servers/browser/server.py'))
    ...
    def test_browser_demo_imports(self):
        ...
        demo_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'examples/agents/browser_demo.py'))
        ...
        with patch('oxygent.MAS') as mock_mas, \
            ...
            with patch.dict(os.environ, env_vars):
                ...
                try:
                    ...
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_oxy_space"), "_create_oxy_space方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_browser_tools"), "_create_browser_tools方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_http_llm"), "_create_http_llm方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_filesystem_tools"), "_create_filesystem_tools方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_browser_agent"), "_create_browser_agent方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_file_agent"), "_create_file_agent方法不存在")
                    self.assertTrue(hasattr(demo_module.BrowserDemo, "_create_master_agent"), "_create_master_agent方法不存在")
                    
                    # 测试load_config函数
                    self.assertTrue(hasattr(demo_module, "load_config"), "load_config函数不存在")
                    
                    # 测试系统提示词常量
                    self.assertTrue(hasattr(demo_module, "MASTER_SYSTEM_PROMPT"), "MASTER_SYSTEM_PROMPT常量不存在")
                    self.assertTrue(hasattr(demo_module, "BROWSER_SYSTEM_PROMPT"), "BROWSER_SYSTEM_PROMPT常量不存在")
                    self.assertTrue(hasattr(demo_module, "FILE_SYSTEM_PROMPT"), "FILE_SYSTEM_PROMPT常量不存在")
    ...
    def test_browser_demo_initialization(self):
        ...
        with patch('oxygent.MAS') as mock_mas, \
            ...
            with patch.dict(os.environ, env_vars):
                ...
                # 模拟BrowserDemo类的方法
                with patch.object(demo_module.BrowserDemo, '_create_oxy_space', return_value=[]) as mock_create_oxy_space:
                    # 初始化BrowserDemo实例
                    try:
                        ...
                        self.assertEqual(browser_demo.config["DEFAULT_LLM_API_KEY"], "test_api_key")
                        self.assertEqual(browser_demo.config["DEFAULT_LLM_BASE_URL"], "https://test.api.com")
                        self.assertEqual(browser_demo.config["DEFAULT_LLM_MODEL_NAME"], "test_model")
                        
                        # 验证方法调用
                        mock_create_oxy_space.assert_called_once()
                        mock_config.set_agent_llm_model.assert_called_once_with("default_llm")
